  ...
  document.getElementById('quran-word-list').innerHTML = items.length
    ? items.map(item => `
      <div class="quran-word-item">
        ...
        <span class="quran-bengali">${item.pronunciation}${item.pronunciationBreakdown ? ` — ${item.pronunciationBreakdown}` : ''}${item.meaning ? ` (${item.meaning})` : ''}</span>
        <span class="quran-ayah">${item.ayahArabic || 'এই শব্দের জন্য আয়াত এখনো যোগ করা হয়নি।'}</span>
